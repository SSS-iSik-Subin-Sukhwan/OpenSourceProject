...
public class FindEatingPlaceResultViewActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    for (int i = 0; i < Values.foodSize; i++) {
      foodListAdapter.addItem(Values.foodList[i], Values.foodAddress[i], Values.foodLat[i], Values.foodLon[i]);
    }
        ...
        new Thread(new Runnable() { // convertGpsToAddress와 kakaoLinkService를 안정적으로 쓰려면 Thread를 써야한다.
          ...
          public void run() {
            try {
              ...
              String kakaoName  = Values.foodList[i]; // 카카오톡에 공유할 때 제목에 사용될 String값을 foodList에 i번에서 가져옴

              String getKakaoAddress; // 주소값을 가져옴

              getKakaoAddress = new TMapData().convertGpsToAddress(Values.foodTMapPoint[i].getLatitude(), Values.foodTMapPoint[i].getLongitude());
              ...
              ButtonObject leftButtonObject = new ButtonObject("앱으로 돌아가기", // 왼쪽 버튼의 표시될 텍스트를 설정
                                                               LinkObject.newBuilder()
                                                                       .setWebUrl("https://map.kakao.com/link/map/"+ Arrays.toString(kakaoAddress) + "," + kakaoName) //
                                                                       .setMobileWebUrl("https://map.kakao.com/link/map/"+ Arrays.toString(kakaoAddress) + "," + kakaoName)
                                                                       .build());

              LocationTemplate params = LocationTemplate.newBuilder(kakaoAddressResult.toString(), // 위치 확인시 보여줄 주소칸에 표시될 값을 설정해줌
                                                                    ContentObject.newBuilder(kakaoName, // 카카오톡 공유시 보여줄 ContentObject를 생성, 제목란에 선택한 주변시설의 명칭을 띄워줌
                                                                                             ...
                                                                                             LinkObject.newBuilder()
                                                                                                     ...
                                                                                                     .setWebUrl("https://map.kakao.com/link/map/"+ kakaoAddressResult + "," + Values.foodTMapPoint[i].getLatitude()+ "," + Values.foodTMapPoint[i].getLongitude()) // 만약 웹에서 버튼들의 기능이 수행이 안된다면 주어진 링크로 이동
                                                                                                     .setMobileWebUrl("https://map.kakao.com/link/map/"+ kakaoAddressResult + "," + Values.foodTMapPoint[i].getLatitude()+ "," + Values.foodTMapPoint[i].getLongitude()) // 만약 모바일에서 버튼들의 기능이 수행이 안된다면 주어진 링크로 이동
                                                                                                     .build())

                                                                            .setDescrption("https://map.kakao.com/link/map/"+ kakaoAddressResult + "," + Values.foodTMapPoint[i].getLatitude()+ "," + Values.foodTMapPoint[i].getLongitude()) // 카카오톡 공유시 설명칸에 하이퍼링크형식으로 웹상에서 지도를 띄워주게 설정
                                                                            .build())
